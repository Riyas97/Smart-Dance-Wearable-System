...
class Server(threading.Thread):
    def __init__(
        self, ip_addr, port_num, group_id, n_moves=len(ACTIONS) * NUM_MOVE_PER_ACTION
    ):
    ...
    def decrypt_message(self, cipher_text):
        ...
        return {"position": position, "action": action, "sync": sync}
    ...
    def set_next_action(self):
        ...
        self.action = self.actions[
            int(index / NUM_MOVE_PER_ACTION)
        ]  # produces indexing error if unchanged
...
def main():
    ip_addr = "ec2-52-221-205-129.ap-southeast-1.compute.amazonaws.com"
    port_num = EVAL_PORT
    group_id = 18
